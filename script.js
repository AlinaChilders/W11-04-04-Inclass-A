...
document.getElementById("onOffButton").addEventListener("click", (event) => {
  ...
  if (!onOff) {
    ...
    document.querySelector("body").style.backgroundColor = "#9E1834";
    setTimeout(() => {
      document.querySelector("body").style.backgroundColor = "#FFFFFF";
    }, 1000); // 1000 = delay time in ms
...
document.getElementById("tempo").addEventListener("input", (event) => {
  ...
  tempo = bpm2ms(event.target.value) / 4; // divided by 4 = 16th nontes
});
// Tempo slider can only be updated before hitting "Start"
